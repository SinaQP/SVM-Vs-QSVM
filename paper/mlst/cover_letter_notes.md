...
The principal result is reported numerically, but the inferential boundary appears immediately beside it: $n=5$, overlapping splits, raw exact Wilcoxon $p=0.0625$, and Holm-adjusted $p=0.1875$. Architecture selection reused the same outer partitions, so the final results are described as potentially optimistic for the selected QSVC architecture rather than as an independent confirmation.
...
The strongest defensible novelty is the integrated design: a frozen WDBC benchmark combining fold-local preprocessing, nested SVM model selection, paired multi-split reporting, ZZ-feature-map ablation, fixed-hyperparameter sample-size analysis, and effective-rank/CKA diagnostics. The result is useful because it identifies a concrete setting in which geometric divergence and increased circuit complexity did not produce predictive advantage, while retaining complete records for independent audit.
...
- Perform the final author read-through of both PDFs and verify that the public `v1.0.0` release remains accessible.
