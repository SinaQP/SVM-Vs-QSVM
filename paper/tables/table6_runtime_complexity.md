...
| **QSVC — PCA 2 / 2Q** | Exact Statevector Matrix Engine (CPU) | 2 | **0.2257** | 0.0232 | $\sim 47\times$ | Gram products: $\mathcal{O}((N_{tr}^2+N_{te}N_{tr})2^q)$, then precomputed-kernel SVC | Train plus test kernels: 2,071,160 bytes |
| **QSVC — PCA 4 / 4Q** | Exact Statevector Matrix Engine (CPU) | 4 | **0.6599** | 0.0863 | $\sim 80\times$ | Same operation-count form with larger $q$ | Train plus test kernels: 2,071,160 bytes |
...
*Note: Source data from `results/final/final_runtime_summary.csv`. Exact statevector and historical `ComputeUncompute` measurements are separate CPU-simulation regimes. Neither is a physical-QPU timing. The 2,071,160-byte value is combined training- and test-kernel storage.*
