...
| **Nested-selected QSVC** | **PCA 2** | **2** | **ZZ Feature Map (reps=1, linear-equivalent; seed-specific $C \in \{1, 10, 100\}$)** | **0.9070 ± 0.0237** | **0.9064 ± 0.0457** | **0.8381 ± 0.0832** | **0.8678 ± 0.0388** | **0.9644 ± 0.0244** | **0.2181 ± 0.0096** |
...
| **Nested-selected QSVC** | **PCA 4** | **4** | **ZZ Feature Map (reps=1, full, $C=1.0$)** | **0.9053 ± 0.0423** | **0.8748 ± 0.0767** | **0.8762 ± 0.0832** | **0.8721 ± 0.0556** | **0.9581 ± 0.0227** | **0.5846 ± 0.0257** |

*Note: Classical rows and their runtime measurements come from `results/final/final_model_comparison.csv`; authoritative QSVC metrics and runtimes come from `results/corrected_nested/qsvc_outer_test_summary.csv`. Classical comparator family/hyperparameters and QSVC architecture/$C$ are selected per seed by five-fold inner cross-validation without outer-test access. At two qubits, linear and full entanglement encode the same single pair and are treated as one canonical topology.*
